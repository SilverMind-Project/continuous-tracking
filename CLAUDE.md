...
class PHRepositoryProtocol(Protocol):
    ...
    async def evidence_backed_commit(self, ph_id: str, identity_id: str, evidence_at: datetime, committed_at: datetime) -> None: ...
...
| File | Contents |
| --- | --- |
| `0001_init` | Complete baseline schema: all tables, hypertables, indexes, triggers, enums, continuous aggregate, refresh and retention policies. Squashes the prior chain (20+ original migrations, then `0002_gait_and_agitation_schema` through `0009_daily_appearance_profiles`). Drop and recreate the database to migrate from an older chain. |
...
**`make ci` is the authoritative gate.** `make check` is the fast local gate; `make check-all` adds Go and proto validation. `make ci` additionally runs `pytest -m integration` which proves C1/C2 (WorldTracker end-to-end) and T5 (PH repository parity) against a testcontainer Postgres. These proofs must pass before merging any change to the tracker or storage layer.
