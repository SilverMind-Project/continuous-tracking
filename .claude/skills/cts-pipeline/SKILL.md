...
| 9 | `ClosePHStage` | `trajectory.py` | Closes PHs that were active in the previous frame but absent in this one (`terminated_ph_ids = prev_active_ph_ids - current_ph_ids`); calls `TrajectoryWriter.close_track` to finalize dwell segments. Defined alongside `TrajectoryStage` in `trajectory.py`, not in `frame_pipeline.py` |
...
The world-tracker integration proofs live in `tests/integration/test_world_tracker_e2e.py` and the
`test_world_tracker_replay_*.py` files. They replay length-prefixed JSON `WorldObservation` fixtures
(`tests/fixtures/frame_replays/*.bin`, generated by `scripts/synthesize_replay_fixture.py`, loaded by
`_load_fixture`) DIRECTLY through `WorldTracker.step`, not through the full frame pipeline (no MinIO,
Triton, detector, face, or pose stages). Note the current loader hardcodes `calibrated=True`, so no
existing proof exercises the uncalibrated synthetic-floor-point path that real home cameras use; extend
the loader and generator to carry `calibrated`, `face_anchor`, and `orientation` when testing those
paths.
