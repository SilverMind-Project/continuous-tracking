...
class PostgresIdentityDecisionRepository:
            ...
            await conn.execute(
                """
                    INSERT INTO continuous_tracking.identity_decisions (
                        ...
                        inferred_identity_id, effective_identity_id, authority,
                        decision_source, conflict_kind, top_probability,
                        second_probability, posterior_entropy,
                        last_independent_evidence_at, config_hash,
                        resolver_version, model_set_version,
    ...
    async def _build_decision(
        ...
        hit_rows = await conn.fetch(
            "SELECT * FROM continuous_tracking.identity_decision_gallery_hits"
            " WHERE decision_id = $1 ORDER BY rank ASC",
            row["decision_id"],
        )
        ...
        return IdentityProvenanceDecision(
            ...
            second_probability=row["second_probability"],
            posterior_entropy=row["posterior_entropy"],
            last_independent_evidence_at=row["last_independent_evidence_at"].replace(tzinfo=UTC)
            if row["last_independent_evidence_at"]
            else None,
            config_hash=row["config_hash"],
            resolver_version=row["resolver_version"],
