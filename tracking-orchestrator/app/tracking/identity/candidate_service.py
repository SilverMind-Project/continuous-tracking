"""Governed ReID candidate lifecycle service.

Creates governed ``reid_gallery`` candidate rows (with immutable MinIO crop
objects) and drives the review state machine: approve, reject, relabel, and
undo. Groundwork for the remaining identity-governance milestones; the
review read path is served by ``app/services/reid_review_service.py``.
"""
...
class ReIDCandidateService:
    ...
    async def create_candidate(
        ...
        # Face-derived candidates require the direct authoritative ArcFace
        # identity to equal both the candidate identity and the resolved
        # effective identity.
        if candidate_reason == "face_derived" and (
            arcface_identity != identity_id or effective_identity != identity_id
        ):
            raise ValueError(
                "identity_mismatch: face identity does not match candidate/effective identity"
            )
        ...
        async with self._gallery_repo._pool.acquire() as conn:
            await conn.execute(
                ...
                (id, identity_id, proposed_identity_id, effective_identity_id, embedding, quality,
                 ...
                 confidence, is_truncated, is_occluded, candidate_reason, source_episode_id,
                 created_actor, origin_tracklet_id, orientation)
                VALUES ($1, $2, $3, $4, $5::vector, $6, 'pending_review', $7, $8, $9, $10, $11, $12,
    ...
    async def _transition_state(
        ...
        async with self._gallery_repo._pool.acquire() as conn, conn.transaction():
            ...
            if new_state == "rejected":
                row = await conn.fetchrow(
                    "SELECT crop_key FROM continuous_tracking.reid_gallery WHERE id = $1",
                    candidate_id,
                )
                if row and row["crop_key"]:
                    try:
                        ...
                        logger.warning(
                            "reid_candidate_crop_delete_failed",
                            candidate_id=candidate_id,
                            crop_key=row["crop_key"],
                        )

    async def approve_candidate(self, candidate_id: str, actor: str, reason: str = "") -> None:
        await self._transition_state(candidate_id, "operator_verified", actor, reason)

    async def reject_candidate(
        self, candidate_id: str, actor: str, reason: str = "", note: str = ""
    ) -> None:
    ...
    async def relabel_candidate(
        self, candidate_id: str, new_identity_id: str, actor: str, reason: str = ""
    ) -> None:
        async with self._gallery_repo._pool.acquire() as conn, conn.transaction():
            row = await conn.fetchrow(
                "SELECT state, audit_version, identity_id"
                " FROM continuous_tracking.reid_gallery WHERE id = $1",
                candidate_id,
            )
            if not row:
                raise ValueError(f"Candidate {candidate_id} not found")
            ...
            await conn.execute(
                ...
                actor,
                reason,
                f"Relabeled from {row['identity_id']}",
                audit_version + 1,
            )

    async def undo_review(self, candidate_id: str, actor: str, reason: str = "") -> None:
        async with self._gallery_repo._pool.acquire() as conn, conn.transaction():
            row = await conn.fetchrow(
                "SELECT state, audit_version FROM continuous_tracking.reid_gallery WHERE id = $1",
                candidate_id,
            ...
            last_event = await conn.fetchrow(
                "SELECT previous_state FROM continuous_tracking.gallery_review_events"
                " WHERE entry_id = $1 ORDER BY event_time DESC LIMIT 1",
                candidate_id,
            )
