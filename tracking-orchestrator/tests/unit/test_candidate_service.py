    ...
    class MockConnection:
        ...
        async def fetchrow(self, sql, *args):
            if "SELECT state, audit_version" in sql:
                ...
                if candidate_id in self.data:
                    return {
                        "state": self.data[candidate_id]["state"],
                        "audit_version": self.data[candidate_id]["audit_version"],
                        "identity_id": "test_id",
                    }
                return None
...
async def test_candidate_eligibility(mock_gallery_repo, mock_storage):
    ...
    with pytest.raises(ValueError, match="not L2 normalized"):
        await service.create_candidate(MockEntity(), "id1", [0.0] * 768, b"crop", None)
    ...
    with pytest.raises(ValueError, match="identity_mismatch"):
        await service.create_candidate(
            ...
            arcface_identity="id2",
            effective_identity="id1",
        )
