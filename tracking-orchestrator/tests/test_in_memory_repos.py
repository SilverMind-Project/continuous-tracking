...
async def test_save_detections(tracking_repo: InMemoryTrackingRepository) -> None:
    ...
    await tracking_repo.save_detections(detections)
...
async def test_save_global_track(tracking_repo: InMemoryTrackingRepository) -> None:
    ...
    t1 = GlobalTrack(global_track_id=track_id, camera_ids=["cam1"], tracklet_ids=["t1"])
    t2 = GlobalTrack(global_track_id=track_id, camera_ids=["cam2"], tracklet_ids=["t2"])
...
async def test_upsert_and_get_gallery_entry(
    ...
    fetched = await gallery_repo.get_gallery_entry("i1")
    assert fetched is not None
...
async def test_search_similar(gallery_repo: InMemoryGalleryRepository) -> None:
    ...
    results = await gallery_repo.search_similar([1.0] * 512, limit=2)
...
async def test_list_activities(activity_repo: InMemoryActivityRepository) -> None:
    await activity_repo.save_activity(
        PersonActivity(identity_id="i1", activity_type="entry", camera_id="c1")
    )
    await activity_repo.save_activity(
        PersonActivity(identity_id="i1", activity_type="exit", camera_id="c2")
    )
