...
| `single_camera_turn.bin` | One person, one uncalibrated camera, turning front-to-back with 6 s occlusion gap (50 steps). Demonstrates PH churn + resolver demotion. Revival target: 1 PH with identity held. | synthetic |
| `cross_camera_handoff.bin` | One person crossing from cam-handoff-a to cam-handoff-b with 16 s gap (63 steps). Proves cross-camera disconnect; cross-camera revival target: identity persists. | synthetic |
| `two_people_one_room.bin` | Two enrolled people (alice + bob) crossing paths on one uncalibrated camera (26 steps). Guardrail: no over-merge. | synthetic |
| `resident_plus_stranger.bin` | Enrolled resident (alice) + unenrolled stranger crossing paths (30 steps). Clinical guardrail: resident identity never transferred to stranger. | synthetic |
