...
- **dedup skips `calibrated=False` observations**, so cross-camera dedup does nothing for uncalibrated cameras; identity then crosses cameras only via the ReID gallery, face propagation, and continuation.
